...
async function preprocessImage(buffer, options = {}) {
    const { maxDimension = 1280, enhanceBrightness = true, enhanceContrast = true, reduceNoise = true, sharpen = true, } = options;
    ...
    const processedMeta = await (0, sharp_1.default)(processedBuffer).metadata();
    return {
 ...
 */
async function extractRegion(buffer, region, targetSize = 640) {
 ...
 */
async function upscaleImage(buffer, scaleFactor) {
    const meta = await (0, sharp_1.default)(buffer).metadata();
