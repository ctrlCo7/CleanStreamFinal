...
exports.runYOLODetection = runYOLODetection;
exports.drawBoundingBoxes = drawBoundingBoxes;
exports.uploadProcessedImage = uploadProcessedImage;
...
async function detectOnBuffer(model, imageBuffer, confidenceThreshold) {
    const tensor = tf.node.decodeImage(imageBuffer, 3);
    let predictions;
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        predictions = await model.detect(tensor, 40, confidenceThreshold);
    }
    finally {
        tensor.dispose();
    }
...
async function runMultiScaleDetection(preprocessedBuffer, model, imageWidth, imageHeight, confidenceThreshold) {
    const all = [];
    const toRaw = (preds, scalePenalty = 1.0) => preds.map((p) => ({
        label: p.class,
    ...
    try {
        const upscaled = await (0, preprocessor_1.upscaleImage)(preprocessedBuffer, 1.5);
        const scale2Preds = await detectOnBuffer(model, upscaled, confidenceThreshold);
        const downscale = (v) => v / 1.5;
        all.push(...scale2Preds.map((p) => ({
            label: p.class,
            wasteType: COCO_TO_WASTE[p.class.toLowerCase()] ?? 'mixed_waste',
            confidence: Math.round(p.score * 100),
            bbox: {
    ...
    for (const region of regions) {
        try {
            const cropBuffer = await (0, preprocessor_1.extractRegion)(preprocessedBuffer, region, 640);
...
// ─── Public API ───────────────────────────────────────────────────────────────
async function runYOLODetection(imageBuffer, confidenceThreshold = 0.25) {
    const startMs = Date.now();
    // Phase 5 — preprocess before inference
    const preprocessed = await (0, preprocessor_1.preprocessImage)(imageBuffer, {
        maxDimension: 1280,
    ...
    const candidates = await runMultiScaleDetection(preprocessed.buffer, model, imageWidth, imageHeight, confidenceThreshold);
...
async function uploadProcessedImage(processedBuffer, userId, imageId) {
