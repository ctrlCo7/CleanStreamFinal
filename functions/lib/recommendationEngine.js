...
exports.estimateTeamNeeded = estimateTeamNeeded;
exports.estimateSpreadArea = estimateSpreadArea;
exports.estimateVolume = estimateVolume;
...
function buildRecommendations(detections) {
    ...
    detections.forEach((det) => {
        ...
        if (det.confidence > existing)
            seen.set(det.wasteType, det.confidence);
    });
...
function computeSeverity(detections, objectCount) {
    if (objectCount === 0)
...
function estimateTeamNeeded(score) {
    if (score >= 75)
        return '5–10 workers';
    if (score >= 50)
        return '3–5 workers';
...
function estimateSpreadArea(objectCount, coverageRatio = 0, imageArea = 307200) {
    if (objectCount === 0)
        return 0;
...
function estimateVolume(objectCount, environmental) {
    if (objectCount === 0)
        return 0;
