...
const QUICK_REPLIES = ['Team dispatched', 'Hazmat en route', 'Maintain 20m distance', 'Cleanup scheduled', 'Thank you!'];
...
export default function AdminChatScreen({ navigation, route }: Props) {
  ...
  const send = useCallback(async (text?: string) => {
    ...
    try {
      ...
      await sendMessage(conversationId, user.uid, senderName, 'admin', receiverId, content, reportTag);
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView ref={scrollRef} contentContainerStyle={styles.chatBody} showsVerticalScrollIndicator={false}>
          ...
          {messages.map((msg) => {
            ...
            return (
              <View key={msg.id}>
                <View style={[styles.bubble, isMe ? styles.bubbleAdmin : styles.bubbleUser]}>
